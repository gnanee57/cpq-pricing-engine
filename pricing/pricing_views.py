...
def pricing():
    if request.method == "POST":
        ...
        if forecast_status is None:
            ...
            timing_risk_monthly = get_preprocessed_file("Timing_Risk_Monthly")
            ...
            validate_input = {
                ...
                "E_TRM_D": (timing_risk_monthly.dataFrameStartDate, timing_risk_monthly.dataFrameEndDate)
            }
            ...
            if status == "IN PROGRESS":
                ...
                timing_risk_monthly_data = pickle.loads(timing_risk_monthly.dataFrame)
                ...
                pre_pricing.remote(mprn_data, unique_mprn_count, price_curve_data_dict, ecn_data, gnt_data, zco_data,
                                   ...
                                   hedging_cost_data, timing_risk_monthly_data,
    ...
    if request.method == "POST":
        ...
        for (idx, rollingRow), (_, customerRow) in zip(df_rolling.iterrows(), df_customer.iterrows()):
            ...
            if status1 and status2:
                ...
                evaluation_model = getSelectedPricingSegmentGroupEvaluationModel(pricingSegmentGroupEvaluationModelId)
                all_versions = evaluation_model.pricing_segment_group_evaluation_model_versions
                version_count = len(all_versions) + 1
                for i in all_versions:
                    i.selected = False

                new_version = PricingSegmentGroupEvaluationModelVersionEO(
                    version=version_count,
                    evaluation_json=json.dumps(updatedEvJSON, cls=NpJsonEncoder),
                    selected=True,
                    pricing_segment_group_evaluation_model=evaluation_model
                )
                evaluation_model.pricing_segment_group_evaluation_model_versions.append(new_version)
                db.session.commit()
            else:
                status = False
...
def getSelectedPricingSegmentGroupEvaluationModel(pricingSegmentGroupEvaluationModelId):
    result = db.session.query(PricingSegmentGroupEvaluationModelEO).filter(
        PricingSegmentGroupEvaluationModelEO.id == pricingSegmentGroupEvaluationModelId).first()
    db.session.commit()
    return result
